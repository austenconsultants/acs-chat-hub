...
const MCP_PATTERNS = {
  ...
  freeswitch: /(?:check\s+)?freeswitch\s+(?:status|health)/i,
  valkey: /(?:check\s+)?valkey\s+(?:cache\s+)?status/i,
  calculate: /calculate\s+(\d+)\s*([+\-*/])\s*(\d+)/i,
  test: /(?:test\s+mcp|mcp\s+test|test\s+connection|mcp\s+connection|mcp)/i,
...
async function callMCPTool(toolName: string, args: any) {
  try {
    ...
    if (toolName === 'list_tools') {
      const response = await fetch('http://10.152.0.71:3210/api/mcp', {
        method: 'POST',
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("[Chat API DEBUG] MCP Settings:", settings?.mcp);
    if (settings?.mcp?.enabled) {
      const mcpCommand = detectMCPCommand(message)
      console.log("[Chat API DEBUG] MCP Command detection result:", mcpCommand);
